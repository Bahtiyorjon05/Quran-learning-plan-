...
export function CovenantWizard({ today }: { today: CivilDate }) {
  ...
  const scope: PlanScope = useMemo(
    () =>
      scopeKind === "full"
        ...
        : {
            kind: "juzRange",
            fromJuz: Math.min(fromJuz, toJuz),
            toJuz: Math.max(fromJuz, toJuz),
          },
  ...
  function setDailyLines(lines: number) {
    ...
    try {
      ...
      const impliedMonths = Math.max(
        ...
        Math.min(
          MAX_MONTHS,
          Math.round(diffDays(today, derived.endDate) / 30.44),
        ),
      );
  ...
  const intensity = !shape
    ? "gentle"
    : shape.dailyPages <= 0.5
      ? "gentle"
      : shape.dailyPages <= 1
        ? "steady"
        : shape.dailyPages <= 3
  ...
  return (
    /* A contained, illuminated step — the same grammar as onboarding, which is
       the screen immediately before this one. Bare text on the ground read as
       something half-rendered rather than as a covenant being written. */
    <div className="panel relative mx-auto w-full max-w-2xl rounded-3xl p-7 sm:p-10">
          ...
          <div className="mt-8">
            ...
            {step === "scope" && (
              <div className="space-y-3">
                ...
                <ScopeCard
                  selected={
                    scopeKind === "juzRange" && fromJuz === 30 && toJuz === 30
                  }
                  onSelect={() => {
                ...
                <ScopeCard
                  selected={
                    scopeKind === "juzRange" && fromJuz === 1 && toJuz === 15
                  }
                  onSelect={() => {
                    setScopeKind("juzRange");
                    setFromJuz(1);
                  ...
                  <div className="mt-5 flex items-end justify-between gap-4 border-t border-[var(--line-subtle)] pt-4">
                    <div>
                      ...
                      <p className="mt-0.5 text-xs text-[var(--text-faint)] tabular-nums">
                        {t("pace.aboutPages", {
                          pages: shape.dailyPages.toFixed(1),
                        })}{" "}
            ...
            {step === "sign" && shape && (
              <form action={submit} className="space-y-6">
                ...
                <input
                  type="hidden"
                  name="fromJuz"
                  value={scope.kind === "juzRange" ? scope.fromJuz : 1}
                />
                <input
                  type="hidden"
                  name="toJuz"
                  value={scope.kind === "juzRange" ? scope.toJuz : TOTAL_JUZ}
                />
                ...
                <dl className="overflow-hidden rounded-2xl border border-[var(--line-strong)]">
                  ...
                  <Row
                    ...
                    value={format.dateTime(
                      new Date(`${shape.endDate}T00:00:00Z`),
                      {
                        day: "numeric",
                        month: "long",
                        year: "numeric",
      ...
      >
        <span
          className={cn(
            ...
            selected
              ? "border-[var(--accent)] bg-[var(--accent-ground)]"
              : "border-[var(--line-strong)]",
          )}
        >
          {selected && (
            <Check
              className="h-3 w-3 text-[var(--on-accent)]"
              strokeWidth={3}
            />
          )}
        </span>
...
function Row({
  ...
  return (
    <div className="flex items-start justify-between gap-6 border-b border-[var(--line-subtle)] px-5 py-3.5 last:border-b-0">
      <dt className="shrink-0 text-[0.8125rem] text-[var(--text-muted)]">
        {label}
      </dt>
      <dd
        className={cn(
